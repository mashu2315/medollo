...
const RandomMedicines = () => {
  ...
  useEffect(() => {
    const fetchRandomMedicines = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/random/med`);
        ...
        setMedicines(data.map(med => ({
          ...
          image_url: med.imageUrl || med.image_url || 'https://placehold.co/160x160/FEE2E2/DC2626?text=No+Image',
          mrp: med.mrp || null,
        ...
        {medicines.map(med => (
          ...
          >
            <img
              ...
              onError={e => { e.target.onerror = null; e.target.src = 'https://placehold.co/160x160/FEE2E2/DC2626?text=No+Image'; }}
            />
            <div className="p-2">
              <h3 className="font-medium text-sm mb-0.5 line-clamp-1">{med.name}</h3>
              ...
              <div className="flex items-center mb-1.5">
                ...
                {med.mrp && (
                  <span className="text-xs text-gray-500 line-through">₹{med.mrp}</span>
                )}
              </div>
